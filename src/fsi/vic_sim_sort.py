...
class VICMediaSimSort(QtCore.QThread):
    ...
    def __dowloadProgress(self, value, maximum, width):
        ...
        self.status.emit('Descargando Configuracion del Modelo... (%.2fMB de %.2fMB)' % (value / 1024 / 1024, maximum / 1024 / 1024))
    ...
    def __prepareKNN(self, n_neighbors: int, totalItems: int):
        ...
        Xidx: int = 0
        for item in self.img_list.values():
    ...
    def run(self):
        self.tInicio = time()
        self.img_list = klepto.archives.file_archive(self.features_file, serialized=True)
